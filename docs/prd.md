...
- **符号支持**：全面支持Martin Fowler《分析模式》附录A "Type Diagrams"中的符号体系：
  ...
  ![基数表示](./diagram-samples/cardinality.png)
    ...
    * **No mapping in this direction（此方向无映射）**
    ...
    > **Derived mapping（派生映射）** 不是一种基数，而是关系线本身的属性：表示该关系由其他关系推导而来，
    > 在图上用杀线 `/` 标记。在 schema 中对应 `relationElement.isDerived: boolean`。
  ...
  ![语义陈述](./diagram-samples/SemanticStatements.png)
    ...
    * **长语义陈述（Long Semantic Statements）**：当规则无法用一个单词概括时，使用"折角便签"符号，根据以下标题开头：
      ...
      > **预留标题**（后续阶段引入）：Instances / Method / Overload 依赖于 Type 拥有属性和操作的能力。
      > 第一阶段 Type 内部只有名称，这三个标题暂不支持，在后续扩展 Type 模型（加入属性/操作）后再加入 schema 和 UI。
...
### 3.3 访问控制
- 无需身份认证，任何知道链接的人均可访问和编辑图表
- 不支持权限控制，所有用户都是编辑者角色

### 3.4 实时协作
> **阶段说明**：第一阶段（MVP）不含协作功能，为纯前端单机编辑（localStorage 持久化）。以下为第二阶段起生效的长期目标：
- 简化协作功能：任何人打开链接即可编辑图表
- 每个会话分配内部随机名称，显示在页面上以便识别当前编辑者
- 支持多人同时编辑，实时同步更改
- 协作模式下不支持离线编辑（第一阶段本地模式天然支持离线）
...
### 4.5 数据存储
- 不需要文件夹或组织分类管理
- 不支持私有图表，所有图表对知道链接的人公开

## 5. 技术架构

> **范围说明**：本章描述的是 **长期目标架构**（包含协作、后端、持久化服务）。
> 第一阶段（MVP）采用 **纯前端 + localStorage** 的简化实现，后端、WebSocket 协作、PostgreSQL 等均推迟到第二/第四阶段。
> 具体阶段划分见第 7 章。
